...
/*** Add MD interface  ***/
//=============video channel [MD->HU]===========
int CTranRecvPackageProcess::sendVideoData(u8*data, u32 size, u32 timeStamp) {
	//set package head type
	setPackageHeadType(MSG_VIDEO_DATA);
	...
	//send package head
	if(!CConnectManager::getInstance()->writeVideoData(sendPackage.packageHead, VIDEO_HEAD_LEN)) {
		return -1;
		}

	//send package data field
	if(!CConnectManager::getInstance()->writeVideoData(data, size)) {
		return -1;
		}
...
//============media channel [MD->HU]=============
int CTranRecvPackageProcess::sendMediaData(u8*data, u32 size, u32 timeStamp) {
	//set package head type
	setPackageHeadType(MSG_MEDIA_DATA);
	...
	//send package head
	if(!CConnectManager::getInstance()->writeMediaData(sendPackage.packageHead, MEDIA_HEAD_LEN)) {
		return -1;
		}

	//send package data field
	if(!CConnectManager::getInstance()->writeMediaData(data, size)) {
		return -1;
		}
...
//============vr channel [MD->HU]=============
int CTranRecvPackageProcess::sendVRReportData(u8* data, u32 size, u32 timeStamp) {
	//set package head type
	setPackageHeadType(MSG_VR_DATA);
...
//=============tts channel [MD->HU]===========
int CTranRecvPackageProcess::sendTTSData(u8*data, u32 size, u32 timeStamp) {
	//set package head type
	setPackageHeadType(MSG_TTS_DATA);
	...
	//send package head
	if(!CConnectManager::getInstance()->writeTTSData(sendPackage.packageHead, TTS_HEAD_LEN)) {
		return -1;
		}

	//send package data field
	if(!CConnectManager::getInstance()->writeTTSData(data, size)) {
		return -1;
		}
